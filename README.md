...
./scripts/install.sh          # 交互式填写配置、生成密钥、可选安装 systemd/MCP
...
```sh
./scripts/uninstall.sh        # 停止服务并可选删除 .venv、.env 及 MCP 配置；保留 data/
```

测试本地 Webhook 签名和入队路径：

```sh
python scripts/send_test_webhook.py
```

脚本默认使用临时 `email_id`，适合验证桥接层能接收并排队 Webhook。要测试完整 Resend 拉取和处理流程，请设置 `RESEND_TEST_EMAIL_ID` 为真实 inbound email ID。
...
通知渠道通过 `NOTIFICATION_TARGET` 设置，例如 `telegram`、`weixin`、`qqbot`、`wecom`、`discord`、`slack`、`signal`。对应平台的凭证在 Hermes（`~/.hermes/.env`）中配置，桥接层通常调用本机 Hermes 的 `hermes send --to <target>`。Telegram 文本通知会优先使用 Bot API Rich Messages 发送，失败时回退到 `hermes send`。
通知文本和成功发送的附件会追加到对应 Hermes 会话上下文中；如果只配置了平台名（如 `telegram`），会选择该平台最近的活动会话。附件以 `MEDIA:<path>` 形式记录，方便后续对话继续引用该文件。
...
- 发送通知时调用 `hermes send --to <target>` 给 Telegram、QQBot 等渠道发消息；Telegram 文本通知会优先尝试 Rich Messages。多媒体附件使用 `MEDIA:<path>`。通知文本和成功发送的附件会写入对应 Hermes 会话的 transcript。
...
python3 -m venv .venv
. .venv/bin/activate
pip install -r requirements-dev.txt   # requirements-dev.txt 已包含生产依赖
```
