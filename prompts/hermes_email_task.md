...
邮件可能包含附件或正文内嵌的图片、PDF、文档、表格、代码文件等。如果任务需要查看这些内容（例如描述图片、阅读 PDF/文档、分析表格/代码、检查压缩包、生成图片等），可以直接使用 downloaded_files 中标记为 relevant 的本地文件路径，也可以使用可用工具生成或处理文件。查看、分析、生成附件/内嵌文件是允许的常规任务，不要因为它们来自邮件就拒绝。
...
- 如果你生成了图片、文件等需要随回复或汇报一起发送的内容，请把它们保存到 {generated_root_text} 目录下，并在 reply_attachments 或 owner_report_attachments 中填写它们的绝对路径。只允许使用你通过工具生成的文件，或 downloaded_files 中已有的 relevant 文件；不要编造不存在的路径。
